...
extern unsigned long base_table[256];

HandShake::HandShake()
{
  random = 0;
  memset(byte1seeds, 0, sizeof(byte1seeds));
  memset(&firstPacket, 0, sizeof(firstPacket));
  memset(&secondPacket, 0, sizeof(secondPacket));
  dwArgs5 = 0;
  dwArgs6 = 0;

  if(table == 0)
  {
          table = new unsigned long[0x10000];
...
void Func_X_2(BYTE *data, DWORD key, BYTE keyByte)
{
    data[0] ^= (data[0] + LOBYTE(LOWORD(key)) + keyByte);
    data[1] ^= (data[1] + HIBYTE(LOWORD(key)) + keyByte);
    data[2] ^= (data[2] + LOBYTE(HIWORD(key)) + keyByte);
    data[3] ^= (data[3] + HIBYTE(HIWORD(key)) + keyByte);
    data[4] ^= (data[4] + LOBYTE(LOWORD(key)) + keyByte);
    data[5] ^= (data[5] + HIBYTE(LOWORD(key)) + keyByte);
    data[6] ^= (data[6] + LOBYTE(HIWORD(key)) + keyByte);
    data[7] ^= (data[7] + HIBYTE(HIWORD(key)) + keyByte);
}
DWORD Func_X_4(DWORD P, DWORD X, DWORD G)
{
    __int64 result = 1;
    __int64 mult = G;
...
void HandShake::setRandomNumber(DWORD value)
{
    random = value;
}

int HandShake::packetReceive(void *data)
{
        tPacket_5000* basepacket = (tPacket_5000*)data;
        if(basepacket->opcode != 0x5000) return 0;

        //First packet
        if(basepacket->size == 0x25 && basepacket->flag == 0xE)
        {
                //Flag 0xE packet
                tPacket_5000_E* packet = (tPacket_5000_E*)data;
                memcpy(&firstPacket, packet, sizeof(tPacket_5000_E));

                DWORD PrivateKey1[2] = {0};
                DWORD PrivateKey2[2] = {0};
                BYTE keyByte = 0;
                ...
                //Setup the security count
                setupCountByte(packet->seedCount);

                //Generate and initialize the private blowfish key
                dwArgs5 = Func_X_4(packet->seedSecurity[3], random, packet->seedSecurity[2]);
                dwArgs6 = Func_X_4(packet->seedSecurity[3], random, packet->seedSecurity[4]);

                //Initialize the blowfish key
                keyByte = LOBYTE(LOWORD(dwArgs6)) & 0x03;
                PrivateKey1[0] = packet->seedSecurity[4];
                PrivateKey1[1] = dwArgs5;
                Func_X_2((LPBYTE)PrivateKey1, dwArgs6, keyByte);
                blowfish.initialize((LPBYTE)PrivateKey1, 8);

                PrivateKey2[0] = dwArgs5;
                PrivateKey2[1] = packet->seedSecurity[4];
                keyByte = LOBYTE(LOWORD(dwArgs5)) & 0x07;
                Func_X_2((LPBYTE)PrivateKey2, dwArgs6, keyByte);

                //Encode the private key
                blowfish.encode((LPBYTE)PrivateKey2, (LPBYTE)PrivateKey2, 8);

                char packetBuffer[32] = {0};
                *(LPWORD)(packetBuffer + 0) = 0xC;
                *(LPWORD)(packetBuffer + 2) = 0x5000;
                *(LPDWORD)(packetBuffer + 6) = dwArgs5;
                *(LPDWORD)(packetBuffer + 10) = *(PrivateKey2 + 0);
                *(LPDWORD)(packetBuffer + 14) = *(PrivateKey2 + 1);
                ...
                //return send(s, packetBuffer, 18, 0);
        }
        //Second packet
        else if(basepacket->size == 0x09 && basepacket->flag == 0x10)
        {
                tPacket_5000_10 * packet = (tPacket_5000_10*)data;
                memcpy(&secondPacket, packet, sizeof(tPacket_5000_10));
                DWORD PrivateKey1[2] = {0};
                BYTE keyByte = 0;

                PrivateKey1[0] = firstPacket.seedSecurity[4];
                PrivateKey1[1] = dwArgs5;

                keyByte = LOBYTE(LOWORD(firstPacket.seedSecurity[4])) & 0x07;
                Func_X_2((LPBYTE)PrivateKey1, dwArgs6, keyByte);
                blowfish.encode((LPBYTE)PrivateKey1, (LPBYTE)PrivateKey1, 8);

                if(PrivateKey1[0] != packet->challenge[0] || PrivateKey1[1] != packet->challenge[1])
                {
                        //MessageBox(0, "ACTIVE_SESSION::_OnMsgReceivedBeforeHandshake() - Handshake ServerSignature Error.", "Fatal Error", MB_ICONERROR);
                        return -1;
                }

                PrivateKey1[0] = firstPacket.seedSecurity[0];
                PrivateKey1[1] = firstPacket.seedSecurity[1];
                Func_X_2((LPBYTE)PrivateKey1, dwArgs6, 0x3);
                blowfish.initialize((LPBYTE)PrivateKey1, 8);
                ...
                *(LPWORD)(packetBuffer + 0) = 12 | 0x8000;
                *(LPWORD)(packetBuffer + 2) = 0x2001;

                WORD size = 9;
                memcpy(packetBuffer + 6, &size, 2);
                strcpy((char*)(packetBuffer + 8), "SR_Client");

                packetBuffer[4] = generateCountByte();
                packetBuffer[5] = generateCheckByte((char*)packetBuffer, 18, firstPacket.seedCRC);

                blowfish.encode(packetBuffer + 2, packetBuffer + 2, 16);
                //DWORD total = blowfish.GetOutputLength(16);
                //return send(s, (char*)packetBuffer, total + 2, 0);
        }

        return 0;
}

int HandShake::packetReceive(void *data, LPBYTE outdata, DWORD *outDataSize)
{
        tPacket_5000* basepacket = (tPacket_5000*)data;
        if(basepacket->opcode != 0x5000) return 0;
        ...
        if(basepacket->size == 0x25 && basepacket->flag == 0xE)
        {
            qDebug() << "Sending first packet";
            qDebug() << "Size is 0x25 and basepacket is 0xE";
                //Flag 0xE packet
                tPacket_5000_E* packet = (tPacket_5000_E*)data;
                memcpy(&firstPacket, packet, sizeof(tPacket_5000_E));
                ...
                *(LPDWORD)(packetBuffer + 14) = *(PrivateKey2 + 1);
                ...
                memcpy(outdata, (char*)packetBuffer, 18);
                *outDataSize = 18;
        }
        //Second packet
        else if(basepacket->size == 0x09 && basepacket->flag == 0x10)
        {
                qDebug() << "Sending second packet";
                qDebug() << "Size is 0x09 and flag is 0x10";
                tPacket_5000_10 * packet = (tPacket_5000_10*)data;
                memcpy(&secondPacket, packet, sizeof(tPacket_5000_10));
                DWORD PrivateKey1[2] = {0};
                BYTE keyByte = 0;
                ...
                *(LPWORD)(packetBuffer2 + 2) = 0x2001;
                ...
                blowfish.encode(packetBuffer2 + 2, packetBuffer2 + 2, 16);
                DWORD total = blowfish.getOutputLength(16);

                // Save the packet
                memcpy(outdata + *outDataSize, packetBuffer2, total + 2);
                *outDataSize += (total + 2);
        }
...
unsigned long HandShake::generateValue(unsigned long* Ptr)
{
...
void HandShake::setupCountByte(unsigned long seed)
{
...
unsigned char HandShake::generateCountByte(void)
{
...
unsigned char HandShake::generateCheckByte(char* packet, int length, unsigned long seed)
{
        if( !packet ) return 0;
